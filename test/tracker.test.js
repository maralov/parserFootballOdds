...
const { computeDerived, computeMarketSignal, computeTableSignal } = require('../src/tracker/derivedFields');

describe('derivedFields', () => {
  test('computeMarketSignal: home favored', () => {
    const sig = computeMarketSignal(0.5, 0.3);
    assert.ok(sig > 0, 'Should be positive for home favorite');
  });

  test('computeMarketSignal: away favored', () => {
    const sig = computeMarketSignal(0.2, 0.55);
    assert.ok(sig < 0, 'Should be negative for away favorite');
  });

  test('computeMarketSignal: returns null if inputs are null', () => {
    assert.equal(computeMarketSignal(null, 0.3), null);
  });

  test('computeTableSignal: positive when home team stronger', () => {
    const standings = {
      home: { pts: 28, mp: 10 },
      away: { pts:  7, mp: 10 },
    };
    const sig = computeTableSignal(standings);
    assert.ok(sig > 0);
    assert.ok(sig <= 1);
  });

  test('computeTableSignal: clamped to [-1, 1]', () => {
    const standings = {
      home: { pts: 30, mp: 10 },
      away: { pts:  0, mp: 10 },
    };
    const sig = computeTableSignal(standings);
    assert.ok(sig >= -1 && sig <= 1);
  });

  test('computeTableSignal: null if standings missing', () => {
    assert.equal(computeTableSignal(null), null);
    assert.equal(computeTableSignal({}), null);
  });

  test('computeDerived: full match with odds', () => {
    const match = {
      odds: { home: 4.53, draw: 4.30, away: 1.63 },
      standings: {
        home: { pts: 7, mp: 10 },
        away: { pts: 28, mp: 10 },
      },
    };
    const d = computeDerived(match);
    assert.ok(d.p1Clean > 0 && d.p1Clean < 1);
    assert.ok(d.p2Clean > 0 && d.p2Clean < 1);
    assert.ok(Math.abs(d.p1Clean + d.pXClean + d.p2Clean - 1) < 0.01, 'Probabilities should sum to ~1');
    assert.ok(d.marketSignal < 0, 'Away is favorite, signal should be negative');
    assert.ok(d.tableSignal < 0, 'Away stronger in table, signal should be negative');
    assert.equal(d.oddsImpliedOver25, null);
  });

  test('computeDerived: null odds → null probabilities', () => {
    const d = computeDerived({ odds: null, standings: null });
    assert.equal(d.p1Clean, null);
    assert.equal(d.marketSignal, null);
    assert.equal(d.tableSignal, null);
  });
});
