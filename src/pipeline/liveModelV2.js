...
/**
 * @param {object} input
 * @param {object} input.match — кандидат з live (id, minute, score, …)
 * @param {object} input.features — buildFeatures + odds уже зовні
 * @param {object|null} input.odds1X2
 * @param {object} input.incidents — scrapeMatchIncidents
 * @param {Array} input.history — з matchSnapshotStore (хронологія, останній = поточний)
 * @param {object} [input.secondHalfSides] — secondHalf з десктопу { home, away }
...
function evaluateLiveModelV2(input) {
  ...
  if (tw === 'before' || tw === 'after') {
    ...
    if (
      ...
      bet = 'UNDER_0_5';
      reason = `ТМ 60–70 v2 | ${statsLine} ${src} | стан=${currentState} | зрізів=${historyLen}`;
    } else {
      reason =
        `60–70 v2 очікування: зрізів=${historyLen}/${LIVE_V2_UNDER_CONFIRM_SNAPSHOTS} burst=${burst} ` +
    ...
    if (overOk && underOk) {
      ...
      if (margin > 0 && Math.abs(pGoal - pDry) < margin) {
        reason = `70–80 v2: tie-break |pG−pD|<${margin} | ${statsLine}`;
      } else {
        bet = pGoal >= pDry ? 'OVER_0_5' : 'UNDER_0_5';
        reason = `${bet === 'OVER_0_5' ? 'ТБ' : 'ТМ'} 70–80 v2 | ${statsLine} ${src}`;
      }
    } else if (overOk) {
      bet = 'OVER_0_5';
      reason = `ТБ 70–80 v2 | ${statsLine} ${src}`;
    } else if (underOk) {
      bet = 'UNDER_0_5';
      reason = `ТМ 70–80 v2 | ${statsLine} ${src}`;
    } else {
      reason = `70–80 v2: немає порогів pG=${pGoal} pD=${pDry} | ${statsLine}`;
    }
  } else if (tw === '80-90+') {
    ...
    if (pGoalDec >= LIVE_V2_PGOAL_MIN_80 || late) {
      ...
      reason = `ТБ 80+ v2 (${tag}${favBonus ? ', фав+' : ''}) | ${statsLine} ${src}`;
    } else if (prevBet === 'UNDER_0_5') {
      reason = `80+ v2 після ТМ: моніторинг (pG_dec=${pGoalDec}) | ${statsLine}`;
    } else {
      reason = `80+ v2: слабкий ТБ pG_dec=${pGoalDec} | ${statsLine}`;
    }
  }
